...
import { Link, LinkType } from 'components/Link.jsx'
 ...
 */
function Text(data) {
  return React.createElement(SimpleWidget, {'value': data})
}
/**
...
function DocumentLink(data) {
  ...
  return {
    ...
    'component': (ctx) => React.createElement(Link, {'type': LinkType.DocumentLink, 'resolveLink': url, 'ctx': ctx})
  }
}
...
function WebLink(data) {
  ...
  const url = data.url;
  return React.createElement(Link, {'type': LinkType.WebLink, 'url': url})
}
...
function FileLink(data) {
  ...
  return React.createElement(Link, {'type': LinkType.WebLink, 'url': url, 'label': file.name})
}
...
function ImageLink(data) {
  ...
  return React.createElement(Link, {'type': LinkType.ImageLink, 'url': url, 'alt': data.image.alt})
}
...
function DateFragment(data) {
   ...
   */
  return React.createElement(SimpleWidget, {'value': new Date(data)})
}
...
function Timestamp(data) {
  ...
  return React.createElement(SimpleWidget, {'value': new Date(correctIso8601Date)})
}
